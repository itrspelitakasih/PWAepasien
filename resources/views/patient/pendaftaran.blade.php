            ...
            @forelse ($bookings as $booking)
                <div class="mb-2 rounded-xl border border-gray-200 p-3 text-sm dark:border-gray-800">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-green-600 dark:text-green-400">Terdaftar</span>
                    </div>
                    <p class="text-gray-500 dark:text-gray-400">
                        {{ $bookingPolis[$booking->kd_poli]?->nm_poli ?? $booking->kd_poli }}
                        &middot;
                        {{ $bookingDokters[$booking->kd_dokter]?->nm_dokter ?? $booking->kd_dokter }}
                    </p>
                    <p class="text-gray-500 dark:text-gray-400">
                        No. reg {{ $booking->no_reg }} &middot; {{ $booking->waktu_kunjungan->format('H:i') }}
                    </p>
                </div>
            @empty
